...
def test_recording_setup():
    ex = Recording(
        param_file=os.path.join(main_dir, "simuran", "params", "default_params.py")
    )
...
def test_nc_recording_loading():
    ...
    ex = Recording(
        param_file=os.path.join(main_dir, "examples", "nc_params.py"),
        base_file=loc,
        load=False,
    )
    ex.signals[11].load()
    ...
    lfp = NLfp()
    lfp.set_filename(
        os.path.join(loc, "01082018_CanCSubCa1_muscimol_smallsq_before_1_1.eeg12")
    )
    lfp.load(system="Axona")

    unit = NSpike()
    unit.set_filename(
        os.path.join(loc, "01082018_CanCSubCa1_muscimol_smallsq_before_1_1.10")
    )
    ...
    spatial.set_filename(
        os.path.join(loc, "01082018_CanCSubCa1_muscimol_smallsq_before_1_1_10.txt")
    )
    spatial.load(system="Axona")

    assert np.all(ex.signals[11].underlying.get_samples() == lfp.get_samples())
    assert np.all(ex.units[5].underlying.get_unit_stamp() == unit.get_unit_stamp())
