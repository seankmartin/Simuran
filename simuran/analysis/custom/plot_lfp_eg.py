...
def main(recording, figures, base_dir):
    ...
    recording_signals = [
        butter_filter(
            lfp.get_samples(),
            recording.signals[0].get_sampling_rate(),
            10,
            1.5,
            90,
            "bandpass",
    ...
    seg_splits = np.arange(in_range[0], in_range[1], 100)
    ...
    for j, split in enumerate(seg_splits[:-1]):
        fig, axes = plt.subplots(
            nrows=len(recording.signals), figsize=(40, len(recording.signals) * 2)
        )
        a = np.round(split, 2)
        ...
        for i, lfp in enumerate(recording_signals):
            ...
            figures.append(
                SimuranFigure(fig, out_name, dpi=100, format="png", done=True)
            )
